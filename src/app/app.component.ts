...
function getScoreColour(countryentitled: boolean, score: number | null, defaultColor = 'LightGray') {

    if (!countryentitled || R.isNil(score) || Number.isNaN(score) || score > 10) {
        return defaultColor;
    }
    if (score <= 2.5) {
        return '#ce181f';
...
export class AppComponent implements OnInit{
  ...
  ngOnInit()
  {
    this.dataService.getCountryData().subscribe(x=>{
    console.log(x);
    this.countryData=x;
    this.loadGlobe();
  })        
  ...
  private loadGlobe() {
    ...
    d3.json('assets/ne_110m_admin_0_countries.json', (err, d) => {
      map.append('g')
        ...
        .attr('d', path)
        .attr('fill', (d: any) => getScoreColour(this.getCountryEntitled(d.properties.ISO_A2),this.getCountryScore(d.properties.ISO_A2)))
        .style('stroke', 'black')
  ...
  private getCountryEntitled(countrycode:string): boolean{
  
    const country=this.countryData[countrycode];
    return country ? country.entitled : false;
  }
